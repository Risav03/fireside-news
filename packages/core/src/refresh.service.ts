...
import { fetchNewsArticles, storeArticles } from "./ingestion.service";
import { processArticles } from "./llm.service";

export const INGEST_GATE_MS = 15 * 60 * 1_000;
export const MAX_ARTICLES_PER_REFRESH = 9;
...
async function ingestAll(prisma: PrismaClient) {
  const newsBatch = await fetchNewsArticles();
...
export async function refreshNews(
  ...
  const contentIds = newArticleIds.length > 0 ? await processArticles(prisma, anthropic, newArticleIds, { limit: MAX_ARTICLES_PER_REFRESH }) : [];
